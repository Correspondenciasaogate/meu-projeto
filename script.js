// ================= CONFIGURAÇÃO =================
const CONFIG = {
    NOME_SISTEMA: "Sistema de Encomendas G&W", 
    ID_CLIENTE: "banco_dados_verde_v1",           
    COR_PRIMARIA: "#16a34a"
};
...
window.onload = () => {
    ...
    // Escuta campos para busca de telefone
...
function buscarContatoAutomatico() {
    ...
    if (agendaMoradores[chave]) {
        ...
        campoTelefone.style.backgroundColor = "#dcfce7";
    } else {
        campoTelefone.value = "";
...
document.getElementById('formRecebimento').addEventListener('submit', function(e) {
    ...
    if (idExistente) {
        ...
        enviarZap(nova, 'chegada'); // ENVIA O WHATSAPP
    }
...
function enviarZap(item, tipo) {
    ...
    let msg = "";
    if (tipo === 'chegada') {
        msg = `Olá, *${item.destinatario}*! 📦\nSua encomenda (NF: *${item.nf}*) chegou no setor de Encomendas.\n*Sala ${item.sala}* (${item.torre}).`;
    } else {
...
function renderizarTabela(dados = encomendas) {
    ...
    ordenados.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button onclick="event.stopPropagation(); editar(${item.id})" style="width:auto; padding:5px; background:#64748b">✏️</button>
                <button onclick="event.stopPropagation(); apagar(${item.id})" style="width:auto; padding:5px; background:#ef4444">🗑️</button>
            </td>
        `;
...
function selecionarUnica(id) {
    ...
    document.getElementById('resultadoConteudo').innerHTML = `
        <div style="background:#f1f5f9; padding:15px; border-radius:8px;">
            ...
            ${item.status === 'Retirado' ? `<img src="${item.assinatura}" style="width:100%; border:1px solid #ddd; background:#fff;" />` : ''}
        </div>
    `;
...
function aplicarFiltros() {
    ...
    const filtrados = encomendas.filter(e => 
        e.sala.toLowerCase().includes(fSala) &&
        e.destinatario.toLowerCase().includes(fNome) &&
